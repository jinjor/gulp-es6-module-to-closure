...
BowerCollector.prototype.collect = function(options) {
  options = options || {};
  var cwd = process.cwd();
  var bowerrcPath = options.bowerrc ? Path.join(cwd, options.bowerrc) : Path.join(cwd, '.bowerrc')
  ...
  fs.readdirSync(directoryPath).forEach(function(dir) {
    ...
    if(fs.statSync(dirPath).isDirectory(dirPath) && fs.existsSync(bowerJsonPath)) {
