...
HeaderExtractor.prototype.traverseImportDeclaration = function(ast, header) {
  ...
  for (var i = 0, len = ast.specifiers.length; i < len; i++) {
    ...
    header.require.push({
      name: importNamespace ? '*' : (isDefualt ? 'default' : specifier.id.name),
      as: (importNamespace || isDefualt) ? specifier.id.name : (specifier.name ? specifier.name.name : null),
      path: path
    });
  }
  return true;
